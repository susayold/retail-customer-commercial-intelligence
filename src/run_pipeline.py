...
def main() -> None:
    ...
    with log_path.open("a", encoding="utf-8") as log_handle:
        ...
        stages: list[tuple[str, list[str]]] = [
            ...
            ("semantic_qa", [sys.executable, "-m", "src.semantic_qa", "--data-root", str(data_root), "--artifact-root", str(artifact_root), "--thresholds", str(thresholds), "--pipeline-run-id", pipeline_run_id, "--drive-root", str(drive_root)]),
            ("semantic_quality_gate", [sys.executable, "-m", "src.enforce_quality_gate", "--artifact-root", str(artifact_root), "--drive-root", str(drive_root), "--require-semantic"]),
        ]
