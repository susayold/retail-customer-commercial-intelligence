...
The preferred execution path is local Python with Google Drive for Desktop (or another explicitly mounted Drive root): code runs on the machine, while raw, curated, warehouse and QA artifacts remain on Drive. A controlled notebook runtime is optional, not required; no step in the repository requires Colab and no raw data is written to the repository or local checkout.
