(() => {
  ...
  const pages = {
    overview: { label: "Overview", title: "Retail Customer & Commercial Intelligence", subtitle: "From Data to Decisions  |  Customers · Products · Promotions · Commercial performance" },
  ...
  const shareLabel = (value, digits = 1) => { if (value == null || Number.isNaN(Number(value))) return "—"; return Number(value) < 0 || Number(value) > 1 ? `⚠ ${pct(value, digits)}` : pct(value, digits); };
  ...
  function basket() {
    ...
    return `${kpis([{ icon: "▣", label: "Panel Net Spend", value: compactMoney(k.panel_net_spend), note: "Observed panel total" }, { icon: "♧", label: "Baskets", value: num(k.baskets / 1000, 1) + "K", note: "Distinct baskets" }, { icon: "▤", label: "Spend per Basket", value: money(k.spend_per_basket, 2), note: "Net spend / basket" }, { icon: "♟", label: "Buying Households", value: num(k.buying_households / 1000, 1) + "K", note: "Observed households" }, { icon: "◇", label: "Category Baskets", value: num(k.category_baskets / 1000, 1) + "K", note: "Category-level baskets" }, { icon: "%", label: "Category Penetration", value: pct(k.category_penetration), note: "Households / panel HH" }])}
      ...
      <div class="grid two">${card(`Category Detail — ${esc(d.selected_category)}`, `${legend(["Selected category spend"])}<div class="chart">${svgLine(d.selected_category_weekly, "spend", { title: `${d.selected_category} weekly category spend`, color: colors[1], source: "Mart_Category_Weekly" })}</div>${note("Click a weekly point or category bar to inspect its source-backed row.")}`)}${card("Cross-Category Pair Affinity", `<div class="chart">${heatmap(d.pair_affinity, d.selected_category)}</div>${note("Attach rate = shared buying households / left-category buying households, derived from Mart_Category_Household. Self-pairs are excluded.")}`)}</div>
      <div class="grid two">${card("Basket Affinity Network", `<div class="chart">${network(d.pair_affinity)}</div>${note("Edges show observed cross-category attach rate; this is a basket association view, not a recommendation or causal network.")}`)}${card("Basket Analysis Guardrail", note("Pairs are built at commodity level with commodity_a < commodity_b, so self-pairs and duplicate directions are excluded. Sparse pairs remain subject to the configured support and basket-count thresholds."), "insight")}</div>`;
  }
  function heatmap(rows) {
    rows = rows.filter((r) => r.left !== r.right); const names = [...new Set(rows.flatMap((r) => [r.left, r.right]))]; if (!names.length) return '<div class="empty">No category pair rows.</div>';
  ...
  function network(rows) {
    rows = rows.filter((r) => r.left !== r.right).sort((a, b) => (b.attach_rate || 0) - (a.attach_rate || 0)).slice(0, 12);
    const names = [...new Set(rows.flatMap((r) => [r.left, r.right]))].slice(0, 8);
    if (!names.length) return '<div class="empty">No cross-category pair rows.</div>';
    const width = 560, height = 250, cx = width / 2, cy = height / 2, radius = Math.min(88, 22 + names.length * 7);
    const points = Object.fromEntries(names.map((name, i) => { const angle = -Math.PI / 2 + i * Math.PI * 2 / names.length; return [name, { x: cx + Math.cos(angle) * radius, y: cy + Math.sin(angle) * radius }]; }));
    let out = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="basket affinity network">`;
    rows.forEach((r) => { const a = points[r.left], b = points[r.right]; if (!a || !b) return; const weight = Math.max(.12, Math.min(1, Number(r.attach_rate) || 0)); out += `<line ${drillAttr({ title: `${r.left} × ${r.right}`, subtitle: "Observed basket association", data: r, source: "Mart_Cross_Category_Pair" })} x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}" stroke="rgba(40,185,133,${.25 + weight * .65})" stroke-width="${1 + weight * 4}"/>`; });
    names.forEach((name) => { const p = points[name]; out += `<circle cx="${p.x}" cy="${p.y}" r="13" fill="#0d6b52"/><text x="${p.x}" y="${p.y + 3}" text-anchor="middle" fill="white" font-size="8">${esc(name.slice(0, 10))}</text>`; });
    return `${out}</svg>`;
  }

  function category() {
    const d = DATA.category, k = d.kpis;
    return `${kpis([{ icon: "◆", label: "Private Label Spend", value: compactMoney(k.private_label_spend), note: "Brand/category mart" }, { icon: "◔", label: "Private Label Share", value: shareLabel(k.private_label_share), note: "Observed mix; ⚠ flags an audited anomaly" }, { icon: "♟", label: "Category Households", value: num(k.category_households / 1000, 1) + "K", note: "Household/category grain" }, { icon: "▣", label: "Top Category Spend", value: compactMoney(k.top_category_spend), note: "Highest commodity" }, { icon: "↗", label: "Brand Mix Coverage", value: pct(k.brand_mix_coverage), note: "Commodity rows covered" }, { icon: "▥", label: "Source Brand Rows", value: num(DATA.meta.source_files.brand_category.rows), note: "Mart_Brand_Category" }])}
  ...
  function promotion() {
    ...
    return `${kpis([{ icon: "◆", label: "Observed Promoted-State Sales Share", value: pct(k.promoted_sales_share), note: "No-promo reference is not assumed" }, { icon: "↗", label: "Product-Store-Weeks", value: compact(k.product_store_weeks), note: "Observed activity rows" }, { icon: "▣", label: "Panel Sales", value: compactMoney(k.panel_sales), note: "Promotion mart" }, { icon: "▥", label: "Avg Sales / Product-Store-Week", value: money(k.sales_per_product_store_week, 2), note: "Descriptive average" }, { icon: "◌", label: "Promotion States", value: num(d.states.length), note: "Source states" }, { icon: "!", label: "Interpretation", value: "ASSOCIATION", note: "No causal lift claim" }])}
      ...
      <div class="grid two">${card("Observed Promoted-State Coverage by Department", `<div class="chart">${svgBar(departmentPromo(d.departments), "observed_coverage", "department", { title: "Observed promoted-state product-store-week share; no-promo reference may be unavailable", format: pct, source: "Mart_Promotion_Category_Week" })}</div>`)}${card("Observational Guardrails", `<div class="callout"><b>Important:</b> Promotion-state results are descriptive association from observed panel sales and preserved product-store-week activity. Promotion assignment is not random; 100% observed coverage is not a no-promo control and cannot support uplift, dependency or margin claims.</div>${note("Click a state, department bar or weekly point for the exact supporting aggregate row.")}`, "insight")}</div>`;
  }
