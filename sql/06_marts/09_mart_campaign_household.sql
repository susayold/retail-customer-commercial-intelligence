...
SELECT
    ...
    e.start_day,
    e.end_day,
    e.pre_28d_observable,
    e.during_observable,
    e.post_14d_observable,
    e.post_28d_observable,
    CASE WHEN e.pre_28d_observable THEN COALESCE(t.pre_28d_spend, 0) END AS pre_28d_spend,
    CASE WHEN e.pre_28d_observable THEN COALESCE(t.pre_28d_baskets, 0) END AS pre_28d_baskets,
    CASE WHEN e.during_observable THEN COALESCE(t.during_spend, 0) END AS during_spend,
    CASE WHEN e.during_observable THEN COALESCE(t.during_baskets, 0) END AS during_baskets,
