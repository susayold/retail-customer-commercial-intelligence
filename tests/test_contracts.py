...
def test_analysis_thresholds_are_centralized():
    thresholds = yaml.safe_load((ROOT / "config/analysis_thresholds.yaml").read_text(encoding="utf-8"))
...
def test_one_command_runner_uses_drive_roots_for_data_artifacts():
    ...
    tokens = (
        ...
        'output=str(qa_root / "storage_status.json")', 'cwd=str(cwd)',
        'cwd=repo_root', 'data_root = args.data_root.resolve()',
        'artifact_root = args.artifact_root.resolve()', 'contracts =',
        'sql_dir =', 'args.thresholds', 'str(thresholds)',
    )
    for token in tokens:
...
def test_makefile_exposes_drive_storage_and_quality_gate_targets():
    ...
    for target in ("schema", "inventory", "profile", "parquet", "warehouse", "segment", "validate", "quality-gate", "stats", "powerbi", "reconcile", "release-audit"):
        target_block = makefile.split(f"{target}:" + chr(10), 1)[1].split(chr(10) + chr(10), 1)[0]
...
def test_release_readiness_document_matches_reconciliation_contract():
    ...
    for token in ("RETAIL_SQL_RECONCILIATION", "RETAIL_BI_RECONCILIATION", "make reconcile", "make release-audit", "v1.0.0", "Drive"):
