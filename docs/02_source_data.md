...
The approximate counts are planning expectations from the supplied blueprint. Replace them with the Drive inventory before publishing metrics. The inventory also records a content SHA-256 for source-file traceability plus planning row-count expectation/delta metadata. A count difference is a review flag, not an automatic rejection.
